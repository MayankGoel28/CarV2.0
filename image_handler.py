...
def update_image(locs, ego_id):
    ...
    id_to_col = [0, "blue", "red", "green", "yellow", "blue", "red", "green", "yellow"]

    background = Image.open("background.png")
    background = background.convert('RGB')


    if 0 in locs and ego_id in locs:
        for x, y in locs[0]:
    ...
    for id in locs:
        if id and ego_id in locs:
            ego_x, ego_y,_,_,_,_,_ = locs[ego_id]
            x, y, t1_x, t1_y, t2_x, t2_y, msg = locs[id]
            x,y = convert_location(x,y, ego_x, ego_y)
            
            if(id!=ego_id and x > 465 and x < 485 and y>465 and y<485):
