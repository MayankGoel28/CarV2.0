...
def update_locs(locs, batch):
    for row in batch:
        id,x,y,t1_x,t1_y = row["ID"], row["x"], row["y"], row["t1_x"], row["t1_y"]
        locs[id] = (x,y,t1_x,t1_y)
    return locs
...
def update_image(locs, ego_id):
    ...
    for id in locs:
        ego_x, ego_y,_,_ = locs[ego_id]
        x, y, t1_x, t1_y = locs[id]
        ...
        draw.line((x+50, y+50, destx+50, desty+50), fill=128, width=10)
